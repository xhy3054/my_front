...
@params     net         网络
            img         输入的图像
            detector    非极大值抑制的网络
            scale_f     每次缩放的尺度（除以这个尺度）
            min_scale   最小缩放尺度
            max_scale   最大缩放尺度
            min_size    缩放后图像较大边的最小尺寸
            max_size    缩放后图像较大边的最大尺寸
'''
def extract_multiscale( net, img, detector, scale_f=2**0.25, 
    ...
    while  s+0.001 >= max(min_scale, min_size / max(H,W)):
        # 如果当前尺寸是小于最大的阈值的，则可以进行提取了
        if s-0.001 <= min(max_scale, max_size / max(H,W)):
            nh, nw = img.shape[2:]
            if verbose: print(f"extracting at scale x{s:.02f} = {nw:4d}x{nh:3d}")
            # extract descriptors 通过网络net提取特征
            ...
            X.append(x.float() * W/nw)  # 在原图中的x坐标集合
            Y.append(y.float() * H/nh)  # 在原图中的y坐标集合
            # 尺度集合
            S.append((32/s) * torch.ones(n, dtype=torch.float32, device=d.device))
            C.append(c) # 将这个尺度下的特征可靠性响应集合放入C序列中
            Q.append(q) # 将这个尺度下的特征可重复性响应集合放入Q序列中
            D.append(d) # 将这个尺度下检测的特征描述子集合放入D序列中
        s /= scale_f  #减小一个尺度

        # down-scale the image for next iteration
        nh, nw = round(H*s), round(W*s) #获得新的图像尺寸
        #使用双线性插值对图像进行缩放
    ...
    Y = torch.cat(Y)    # 将Y中不同尺度下的y坐标tensor拼接起来放入一个大tensor中
    X = torch.cat(X)    # 将X这个大序列变成tensor类型
    S = torch.cat(S) # scale
    # 拼接成tensor后，对应值相乘得到对应点的综合分数
    scores = torch.cat(C) * torch.cat(Q) # scores = reliability * repeatability
    XYS = torch.stack([X,Y,S], dim=-1)  # stack函数是将对应维度的拼接起来放入一个tensor（-1表示最后一个维度），此处表达的意思是将每个特征的x,y,s拼起来成一个小tensor
    D = torch.cat(D)    
    return XYS, D, scores   # 返回关键点的位置信息，描述子，评分        
