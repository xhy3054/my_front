...
class NghSampler2 (nn.Module): #基类是网络基类，所以此类其实是一种网络
    ...
    def precompute_offsets(self):
        ...
        for j in range(-rad, rad+1, self.sub_d):
          for i in range(-rad, rad+1, self.sub_d):
            d2 = i*i + j*j
            if d2 <= pos_d2:
                pos.append( (i,j) ) #正确的偏移
            elif neg_d2 <= d2 <= rad2:  #错误的偏移
                neg.append( (i,j) )
    ...
    '''
    @function 采样函数，在输入的图像上采样点，有两种做法：第一种是根据步长均匀采样，第二种是随机采样 
    @param 
        @input1 step:一个整数，表示采样的频率
        @input2 aflow:输入的一对图片的真值flow
    '''
    def gen_grid(self, step, aflow):
        ...
        if step > 0:
            # regular grid 均匀的取值，在图像尺寸范围内
            # 直接在图像范围上根据步长均匀的生成采样点
            x1 = torch.arange(self.border, W-self.border, step, device=dev) #采样点的x坐标
            y1 = torch.arange(self.border, H-self.border, step, device=dev) #采样点的y坐标
            H1, W1 = len(y1), len(x1)   #这种采样可以能x和y的采样数目不同
            # 扩展采样点，将x重复y数目的次数，将y数目重复x数目的次数，使得x,y数目相同。然后再都重复B次，最后变成1列数据
            x1 = x1[None,None,:].expand(B,H1,W1).reshape(-1)
            y1 = y1[None,:,None].expand(B,H1,W1).reshape(-1)
            b1 = b1[:,None,None].expand(B,H1,W1).reshape(-1)
            shape = (B, H1, W1)
        else:
            # randomly spread 在图像尺寸范围内，随机取值
            n = (H - 2*self.border) * (W - 2*self.border) // step**2    #采样的个数
            x1 = torch.randint(self.border, W-self.border, (n,), device=dev)    #采样点的x坐标
            y1 = torch.randint(self.border, H-self.border, (n,), device=dev)    #采样点的y坐标
            # 扩展数据集，此处是将上述的x1之类的序列重复B次，B是图像数量，最后变成一列数据。
    ...
    def forward(self, feats, confs, aflow, **kw):
        ...
        # b1,y1,x1是随机生成的图像大小边界内的数组成的tensor，尺寸为shape
        # 这一步就是在图像上进行取值确定位置，取某个点的值
        b1, y1, x1, shape = self.gen_grid(self.sub_q, aflow)

        # sample features from first image
        # 获得上面确定的位置的值
        feat1 = feat1[b1, :, y1, x1]    # 获得这些点的描述子
        qconf = conf1[b1, :, y1, x1].view(shape) if confs else None # 获得这些点的可靠性
        
        #sample GT from second image
        b2 = b1
        # 获得这些点的绝对光流
        xy2 = (aflow[b1, :, y1, x1] + 0.5).long().t()
        # 通过判断这些点的光流的流动范围是否在图像内 确定掩膜
        mask = (0 <= xy2[0]) * (0 <= xy2[1]) * (xy2[0] < W) * (xy2[1] < H)
        mask = mask.view(shape) #对掩膜的形状加以规范
        ...
        # compute positive scores 计算正样本的分数
        xy2p = clamp(xy2[:,None,:] + self.pos_offsets[:,:,None])    # 第二幅图中每个点的真值位置 加上 29个偏移量,得到n*29个新的位置
        # 计算余弦相似度，所以其实就是两个描述子相乘，img1中的每个点与img2中其真值点附近的29个点的描述子相乘，得到n*29个余弦相似度
...
#        pscores = (feat1[None,:,:] * feat2p[b1,:,xy1p[1], xy1p[0]]).sum(dim=-1).t()
        if self.maxpool_pos:
            pscores, pos = pscores.max(dim=1, keepdim=True) #获得余弦距离最近的分数和位置
            if confs:   #如果还存在可靠性层
                sel = clamp(xy2 + self.pos_offsets[:,pos.view(-1)]) #获得真值点附近余弦相似度最大的位置
                qconf = (qconf + conf2[b2, :, sel[1], sel[0]].view(shape))/2    #获得这些点在两帧上的可靠性分数的平均值
        
        # compute negative scores 计算负样本的分数
        xy2n = clamp(xy2[:,None,:] + self.neg_offsets[:,:,None])    #通过负样本偏移，获得80个负样本位置
        nscores = (feat1[None,:,:] * feat2[b2, :, xy2n[1], xy2n[0]]).sum(dim=-1).t()    #计算这些点的匹配的负样本的分数，即余弦相似度

        if self.sub_d_neg:
            # add distractors from a grid 随机生成一些新的点
            b3, y3, x3, _ = self.gen_grid(self.sub_d_neg, aflow)
            distractors = feat2[b3, :, y3, x3]  #获得这些点的描述子
            dscores = torch.matmul(feat1, distractors.t())  #计算随机点到原本点之间的余弦相似度
            del distractors
            
            # remove scores that corresponds to positives or nulls
            dis2 = (x3 - xy2[0][:,None])**2 + (y3 - xy2[1][:,None])**2  #新生成的点到真值匹配点的欧式距离
            dis2 += (b3 != b2[:,None]).long() * self.neg_d**2   #加上层间距离？
            dscores[dis2 < self.neg_d**2] = 0   #将距离小于阈值的点的分数设为0
            
            scores = torch.cat((pscores, nscores, dscores), dim=1)  #将三种 正样本分数、负样本分数、随机样本分数 拼接到一起
        else:
            # concat everything
