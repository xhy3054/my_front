...
void SPDetector::operator()( cv::Mat _image, cv::InputArray _mask, std::vector<cv::KeyPoint>& _keypoints, cv::Mat _descriptors)
{
    //cv::Mat SPdetect(std::shared_ptr<SuperPoint> model, cv::Mat img, std::vector<cv::KeyPoint> &keypoints, double threshold, bool nms, bool cuda)

    getKeyPoints(0.015, 16, 640-16, 16, 400-16, _keypoints, true);
}
