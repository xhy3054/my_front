...
bool VideoStreamer::next_frame()
{
    ...
    if(img_source == input_device::IS_CAMERA || img_source == input_device::IS_VIDEO_FILE){
        ...
        if(pImgSize != NULL)
        {
            ...
            cv::resize(input, input, (*pImgSize), 0., 0., cv::INTER_LINEAR);
        }

        //input.convertTo(input, CV_32F);        //注意：输入网络的图像数据类型应该是浮点型
    }
    else
    {
