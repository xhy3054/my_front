...
string file_1 = "../of/1.png";  // first image
string file_2 = "../of/2.png";  // second image
...
int main(int argc, char **argv) {
    ...
    vector<Point2f> kpt1;
    for (auto &kp: kp1) kpt1.push_back(kp.pt);
    // now lets track these key points in the second image
    // first use single level LK in the validation picture
    vector<cv::Point2f> kp2_single;
    vector<uchar> success_single;
    OpticalFlowSingleLevel(img1, img2, kpt1, kp2_single, success_single);

    // then test multi-level LK
    vector<cv::Point2f> kp2_multi;
    vector<uchar> success_multi;
    OpticalFlowMultiLevel(img1, img2, kpt1, kp2_multi, success_multi);
